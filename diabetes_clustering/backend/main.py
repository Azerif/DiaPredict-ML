...
class PredictionInput(BaseModel):
    ...
    def normalize_gender(cls, v):
        ...
        for key, value in GENDER_MAPPING.items():
            if v_clean == key.lower():
                return value
        raise ValueError(f'Invalid gender. Accepted: {list(GENDER_MAPPING.keys())}')
    ...
    def normalize_smoking(cls, v):
        ...
        for key, value in SMOKING_MAPPING.items():
            if v_clean == key.lower():
                return value
        raise ValueError(f'Invalid smoking_history. Accepted: {list(SMOKING_MAPPING.keys())}')
...
class ClusterPredictor:
    ...
    def set_preprocessors(self, preprocessors):
        ...
        # Get feature names
        try:
            self.onehot_features = self.onehot_encoder.get_feature_names_out(['smoking_history'])
        except:
            num_cats = len(self.onehot_encoder.categories_[0])
            self.onehot_features = [f"smoking_history_cat{i}" for i in range(num_cats)]
        
        self.feature_order = ['gender'] + self.numeric_features + list(self.onehot_features)
        print(f"✅ Preprocessors loaded. Features: {len(self.feature_order)}")

    def predict(self, gender, age, hypertension, heart_disease, bmi, HbA1c_level, blood_glucose_level, smoking_history):
        # Create DataFrame
        data = pd.DataFrame({
            'gender': [gender], 'age': [float(age)], 'hypertension': [int(hypertension)],
            'heart_disease': [int(heart_disease)], 'bmi': [float(bmi)],
            'HbA1c_level': [float(HbA1c_level)], 'blood_glucose_level': [float(blood_glucose_level)],
            'smoking_history': [smoking_history]
        })
        
        # Preprocess
        data['gender'] = self.label_encoder.transform(data['gender'])
        smoking_encoded = self.onehot_encoder.transform(data[['smoking_history']])
        smoking_df = pd.DataFrame(smoking_encoded, columns=self.onehot_features, index=data.index)
        data = pd.concat([data.drop(columns=['smoking_history']), smoking_df], axis=1)
        data[self.numeric_features] = self.scaler.transform(data[self.numeric_features])
...
async def predict_cluster(input_data: PredictionInput):
    ...
    if input_data.gender not in gender_categories:
        raise HTTPException(status_code=400, detail=f"Gender must be one of: {gender_categories}")
    if input_data.smoking_history not in smoking_categories:
        raise HTTPException(status_code=400, detail=f"Smoking history must be one of: {smoking_categories}")
    
    try:
        predicted_cluster, probabilities = predictor.predict(
            input_data.gender, input_data.age, input_data.hypertension,
            input_data.heart_disease, input_data.bmi, input_data.HbA1c_level,
            input_data.blood_glucose_level, input_data.smoking_history
        )
